...
namespace Microsoft.ILP2025.EmployeeCRUD.Servcies
{
    ...
    public async Task CreateEmployee(EmployeeEntity employee) => await employeeRepository.CreateEmployee(employee);

    public async Task UpdateEmployee(EmployeeEntity employee) => await employeeRepository.UpdateEmployee(employee);
